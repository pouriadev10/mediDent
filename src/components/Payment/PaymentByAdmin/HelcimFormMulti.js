    ...
    const checkStatus = async () => {
        ...
        fetch(config.apiGateway.URL + "/billpay/create-installment/"
            , {
                ...
                body: JSON.stringify({
                    "interval_count":localStorage.getItem("inter") && localStorage.getItem("inter").length == 1 ? localStorage.getItem("inter") : localStorage.getItem("wizard_recurring_interval_count"),
                    "payment_request": props.paymentId,
                }),
            })
            .then((res) => res.json())
            .then((data) => {
